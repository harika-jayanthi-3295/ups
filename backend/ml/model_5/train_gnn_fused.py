"""Train model_5 (fused RSSI + phase/channel GNN) -> artifacts/gnn_fused.pt.

Hyperparameters match model_3's train_gnn_pc.py so the comparison isolates the
fused feature set.
"""
...
from gnn_fused_model import GNNFused, build_edge_index, build_graphs, save_gnn_fused
...
def main() -> None:
    ...
    model = GNNFused(len(ds.antenna_order), hidden=HIDDEN, emb_dim=EMB_DIM)
    ...
    path = ARTIFACTS_DIR / "gnn_fused.pt"
    save_gnn_fused(model, ds.scaler, ds.antenna_order, xyz, xyz_norm, edge_index, HIDDEN, EMB_DIM, path)

    from gnn_fused_model import load_gnn_fused, predict_gnn_fused

    bundle = load_gnn_fused(path)
    preds = predict_gnn_fused(bundle, ds.train)
