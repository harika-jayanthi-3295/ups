"""Step-2: GNN over the antenna graph for tag zone prediction.

Each tag becomes a graph on the 32 antenna nodes. Node features combine the
antenna's static position with that tag's read statistics at the antenna. Edges
connect physically-close antennas and antennas on the same reader, so message
passing lets a node reason about its neighbours' readings. A graph-level
readout feeds four classification heads (region, row, reader, antenna).
"""
...
from features import DYN_DIM, FeatureScaler, xyz_norm_stats
from dataset import Sample

NODE_STATIC_DIM = 3  # normalized x, y, z
NODE_FEAT_DIM = DYN_DIM + 1 + NODE_STATIC_DIM  # dyn + present + xyz
...
def sample_to_graph(
    ...
    xyz_norm: tuple[np.ndarray, np.ndarray],
    xyz: np.ndarray,
    edge_index: torch.Tensor,
) -> Data:
    mean, std = xyz_norm
    scaled = scaler.transform(sample.dyn, sample.present)          # [32, DYN]
...
class GNN(nn.Module):
    def __init__(self, n_antennas: int, hidden: int = 96, emb_dim: int = 16):
        ...
        self.heads = nn.ModuleDict(
            {name: nn.Linear(trunk, n_cls) for name, n_cls in HEADS}
        )
...
def save_gnn(model, scaler, antenna_order, xyz, xyz_norm, edge_index, hidden, emb_dim, path):
    torch.save(
        {
...
def load_gnn(path):
    ...
    model = GNN(len(ckpt["antenna_order"]), hidden=ckpt["hidden"], emb_dim=ckpt["emb_dim"])
...
@torch.no_grad()
def predict_gnn(bundle, samples, batch_size: int = 256):
